...
# With Reproductive Compensation + Inbreeding via Sib-Matijng
...
# Due to the # of total parameter combinations and our compute resources
# we loop over the C parameter within this script
# to keep the number of jobs submitted within our allowance
for (C in seq(0,1,by=0.1)) {
	#print(C) # Used to keep track of loop
  ...
  # Generate Empty Table for Results Each Generation
	genotype_frequencies <- matrix(0,nrow=100000,ncol=12) 
	genotype_frequencies[gen,] <- c(s,t,k,f,C,gen,AA_current,Aa_current,aa_current,Aal_current,aal_current,alal_current)
  ...
  # Loop through until an equilibrium frequency for the male-sterile allele is reached
	while (gen < 100 | a_freq_dif > (1/10000)) {
      ...
      # Proportion of Sib-Mating Relies on Knowing Prob Brother/Sis Genos
      	current_mating_table <- three_allele_generate_mating_table_with_compensation_inbreeding(AA_current,Aa_current,aa_current,Aal_current, aal_current, alal_current,k,s,t,f,C,prob_brothergeno_given_female_offspring_geno=current_prob_brothergeno_given_female_offspring_geno)
    	}
      ...
      # the probability of brother's geno given female geno
    	current_prob_brothergeno_given_female_offspring_geno <- three_allele_calculate_inbreeding_genotype_probabilities(mating_table=current_mating_table,geno_freq_next_gen=genotype_freq_next_gen)
    	...
    	genotype_frequencies[gen,] <- c(s,t,k,f,C,gen,AA_nextgen,Aa_nextgen,aa_nextgen,Aal_nextgen,aal_nextgen,alal_nextgen)
...
# A > 0.01 and a > 0.01
  ...
  genotype_frequencies <- genotype_frequencies[genotype_frequencies[,6]!=0,]
  ...
  colnames(genotype_frequencies) <- c("s","t","k","f","C","gen","AA_freq","Aa_freq","aa_freq","Aal_freq","aal_freq","alal_freq","introduce_al")

  filename <- paste('Model4=',t,'_k=',k,'_f=',f,'.txt',sep='')
  output_path <- paste('/path/to/Model4/results/',filename,sep='')
